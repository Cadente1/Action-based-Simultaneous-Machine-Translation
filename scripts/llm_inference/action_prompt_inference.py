...
"""Run full-sentence action-prompt inference with a Qwen-style causal LM.

This script is for full-sentence action-aware inference. It does not simulate
online prefix-by-prefix decoding. The model receives the complete source
sentence, the allowed action set, and optional language-specific action
statistics, then outputs the final translation.

The action statistics file is usually generated by:
    scripts/gpt_batch/aggregate_action_metrics.py
...
DEFAULT_ALLOWED_ACTIONS = "READ, WRITE, DROP, CUT, PRONOUN, PARTIAL_SUMMARIZATION"

DEFAULT_ACTION_STATISTICS = (
    "No action-level development-set statistics are provided. "
    "Choose actions according to the action definitions and the source sentence."
)
...
Keep the original meaning and preserve the source word order as much as possible when translating.
...
def load_action_statistics(path: Optional[str]) -> str:
    if path is None:
        return DEFAULT_ACTION_STATISTICS

    stats_path = Path(path)
    if not stats_path.exists():
        raise FileNotFoundError(f"Action statistics file not found: {stats_path}")

    if stats_path.suffix.lower() == ".json":
        data: Dict[str, Any] = json.loads(stats_path.read_text(encoding="utf-8"))
        language_pair = data.get("language_pair", "unknown language pair")
        actions = data.get("actions", {})
        lines = [f"Based on development-set evaluation for {language_pair}:"]
        for action, values in actions.items():
            bleu = values.get("bleu", "N/A")
            laal = values.get("laal", "N/A")
            lines.append(f"- {action} -> LAAL ≈ {laal}s, BLEU ≈ {bleu}")
        return "\n".join(lines)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run full-sentence Qwen action-prompt inference for simultaneous interpretation."
    )
    ...
    parser.add_argument(
        "--action_statistics_file",
        default=None,
        help="Optional .txt or .json file containing language-specific per-action BLEU/LAAL statistics.",
    )
...
def main() -> None:
    ...
    action_statistics = load_action_statistics(args.action_statistics_file)
    ...
    if args.action_statistics_file:
        print(f"[info] Loaded action statistics from: {args.action_statistics_file}")
